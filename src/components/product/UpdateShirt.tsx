...
import { doc, getDoc, updateDoc, collection, getDocs, query, orderBy, limit, getFirestore } from "firebase/firestore"
...
import { AlertDialog, AlertDialogContent, AlertDialogDescription, AlertDialogHeader, AlertDialogTitle, AlertDialogTrigger} from "../ui/AlertDialog"
...
const damaSizes = ['XS', 'S', 'M', 'L', 'XL', 'XXL']
const hombreSizes = ['XS', 'S', 'M', 'L', 'XL', 'XXL', 'XXXL', 'XXXXL']
const niñoSizes = ['XS', 'S', 'M', 'L', 'XL']

function UpdateShirt({ firebaseConfig ,companyId, warehouseId, productId }: UpdateShirtProps) {
  ...
  const [, setProductBoxNumber] = useState(0)
  const [, setLastUsedProductNumbers] = useState<{ [key: string]: number }>({})
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      ...
      try {
        const productDoc = await getDoc(doc(db, `companies/${companyId}/warehouses/${warehouseId}/shirts`, productId))
        if (productDoc.exists()) {
          ...
          Object.values(productData.sizes).forEach((size) => {
            size.barcodes.forEach((barcode) => {
              const boxNumber = barcode.slice(7, 13)
              const productNumber = Number.parseInt(barcode.slice(13))
    ...
    const fetchLastBarcode = async () => {
      try {
        const productsRef = collection(db, `companies/${companyId}/warehouses/${warehouseId}/shirts`)
        ...
        if (!querySnapshot.empty) {
          ...
          if (lastBarcode) {
            ...
            const boxNum = Number.parseInt(lastBarcode.slice(7, 13))
            const prodNum = Number.parseInt(lastBarcode.slice(13))
            setBoxNumber(boxNum)
  ...
  useEffect(() => {
    const fetchCustomBrands = async () => {
      const brandsRef = collection(db, `companies/${companyId}/brands/shirt/items`)
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
    ...
    if (e.target instanceof HTMLTextAreaElement) {
      e.target.style.height = "40px"; // Reinicia la altura
  ...
  const getUniqueFileName = async (originalName: string) => {
    const storageRef = ref(storage, `companies/${companyId}/warehouses/${warehouseId}/shirts`)
  ...
  const handleAddSize = async () => {
    if (newSize && newQuantity > 0 && product) {
      ...
      let currentBoxNumber = Number.parseInt(lastUsedBarcode.slice(7, 13))
      let currentProductNumber = Number.parseInt(lastUsedBarcode.slice(13))
      ...
      for (let i = 0; i < newQuantity; i++) {
        ...
        newBarcodes.push(`${prefix}${dateString}${boxString}${productString}`)
      }
      ...
      try {
        await updateDoc(
          doc(db, `companies/${companyId}/warehouses/${warehouseId}/shirts`, product.id),
          updatedProduct,
        )
 ...
 const handleAddBarcode = (size: string) => {
    ...
    const lastUsedBarcode =
        Object.values(product.sizes)
            .flatMap((sizeData) => sizeData.barcodes)
            .sort((a, b) => b.localeCompare(a))[0] || product.barcode;

    const currentBoxNumberBase = Number.parseInt(lastUsedBarcode.slice(7, 13));
    const currentProductNumberBase = Number.parseInt(lastUsedBarcode.slice(13));

    let currentBoxNumber = currentBoxNumberBase;
    let currentProductNumber = currentProductNumberBase + 1;
    ...
    const boxString = currentBoxNumber.toString().padStart(6, "0");
    const productString = currentProductNumber.toString().padStart(2, "0");
    const newBarcode = `${prefix}${dateString}${boxString}${productString}`;

    const allBarcodes = Object.values(product.sizes).flatMap((sizeData) => sizeData.barcodes);
    ...
    setProduct((prev) => {
        ...
        // Verificar si el barcode ya está en la lista antes de agregarlo
        if (updatedSizes[size].barcodes.includes(newBarcode)) {
            return prev; // No actualizar si ya está
        }

        updatedSizes[size].quantity += 1;
        updatedSizes[size].barcodes = [...updatedSizes[size].barcodes, newBarcode];
    ...
    setLastUsedProductNumbers((prev) => ({
        ...prev,
        [boxString]: currentProductNumber,
    }));
    setProductBoxNumber(currentBoxNumber);
};
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 📌 Si hay una nueva imagen, subimos la imagen con `uploadBytesResumable`
      if (newImageFile) {
        ...
        try {
          ...
          const imagePath = `companies/${companyId}/warehouses/${warehouseId}/shirts/${uniqueFileName}`;
  
          // Crear referencia de almacenamiento
          const storageRef = ref(storage, imagePath);
  
          // Subir la imagen con reanudación automática
          const uploadTask = uploadBytesResumable(storageRef, newImageFile);
  
          // Monitorear la subida y obtener la URL al finalizar
          ...
          // Guardamos la nueva URL en el objeto `updatedProduct`
      ...
      await updateDoc(doc(db, `companies/${companyId}/warehouses/${warehouseId}/shirts`, product.id), updatedProduct);
  
      // Actualizar estado del producto con los precios formateados
      setProduct({
        ...updatedProduct,
  ...
  return (
    <div className="min-h-screen bg-blue-100 dark:bg-gray-600 pb-16">
      <header className="bg-teal-600 text-white p-3 flex items-center">
        ...
        <h1 className="text-xl font-bold flex-grow">Update {isBox ? "Box" : "Shirts"}</h1>
        {isBox && (
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <Textarea id="comments" name="comments" value={product.comments} onChange={handleInputChange} className="w-full min-h-[40px] max-h-60 resize-y overflow-auto"/>
                </div>
                        ...
                        <AlertDialog>
                          ...
                          <AlertDialogContent className="max-w-3xl">
                            ...
                            <div className="flex justify-center space-x-2">
                              <Button onClick={(e) => { e.preventDefault(); handleAddBarcode(size); }}>
              ...
              <div className="flex justify-end space-x-2">
                <Button
                  type="button"
                  variant="outline"
                  onClick={() => window.location.href = `/companies/${companyId}/warehouses/${warehouseId}/shirt-inventory`}
                >
                  Cancel
                </Button>
...
export default withPermission(UpdateShirt, ["create"]);